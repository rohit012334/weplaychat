...
import { createFrame, updateFrame } from "@/store/frameSlice";
import { baseURL } from "@/utils/config";
import { DangerRight } from "@/api/toastServices";

const ACCEPTED_TYPES = ["gif", "mp4", "svga"] as const;
type FileType = (typeof ACCEPTED_TYPES)[number];

const MAX_MP4_DURATION = 30; // seconds
...
const FrameDialog = () => {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!type) {
            DangerRight("Only GIF, MP4, and SVGA files are allowed.");
            return;
        }

        // Enforce 30s limit for MP4
        if (type === "mp4") {
            const duration = await getMp4Duration(selected);
            if (duration > MAX_MP4_DURATION) {
                DangerRight(`MP4 must be ${MAX_MP4_DURATION} seconds or less. Your video is ${Math.round(duration)}s.`);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!mongoId && !file) {
            DangerRight("Please select a file.");
            return;
        }
        ...
        if (fileType === "svga") {
            return (
                <div style={{ color: "#8F6DFF", fontSize: "13px", fontWeight: 500, textAlign: "center" }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="#8F6DFF" viewBox="0 0 16 16">
                                    ...
                                    <div className="col-12">
                                        ...
                                        {filePreview && (
                                            <button
                                                ...
                                                style={{ marginTop: "6px", background: "none", border: "none", color: "#dc3545", fontSize: "12px", cursor: "pointer", padding: 0 }}
                                            >
                                                ✕ Remove file
                                            </button>
                                        )}
