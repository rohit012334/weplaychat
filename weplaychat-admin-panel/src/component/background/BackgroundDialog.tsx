...
const BackgroundDialog = () => {
    ...
    const { isLoading } = useSelector((state: RootStore) => state.background);
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!type) { DangerRight("Only MP4, JPG, and PNG files are allowed."); return; }
        if (type === "mp4") {
                    ...
                    <div className="col-xl-4 col-md-6 col-11">
                        <div className="mainDiaogBox">
                            <div className="row justify-content-between align-items-center formHead">
                                <div className="col-8"><h4 className="text-theme m0">{mongoId ? "Edit Background" : "Add Background"}</h4></div>
                                ...
                                <div className="row align-items-start formBody">
                                    <div className="col-12">
                                        <label className="form-label" style={{ fontWeight: 600, fontSize: "14px", color: "#555" }}>
                                            Background File
                                            <span style={{ color: "#8F6DFF", fontSize: "12px", fontWeight: 400, marginLeft: "8px" }}>(MP4, JPG, PNG)</span>
                                        </label>
                                        <div onClick={() => fileInputRef.current?.click()} style={{ border: "2px dashed #8F6DFF", borderRadius: "12px", padding: "12px", cursor: "pointer", textAlign: "center", background: "#faf8ff", minHeight: "150px", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "8px" }}>
                                            {filePreview ? (
                                                fileType === "mp4" ? (
                                                    ...
                                                    <img src={filePreview} alt="Preview" style={{ maxWidth: "100%", maxHeight: "150px", borderRadius: "8px", objectFit: "cover" }} />
                                                )
                                            ) : (
                                                <>
                                                    ...
                                                    <span style={{ color: "#8F6DFF", fontSize: "13px", fontWeight: 500 }}>Click to upload MP4 / JPG / PNG</span>
                                                </>
                                            )}
                                        </div>
                                        <input ref={fileInputRef} type="file" accept=".mp4,.jpg,.jpeg,.png" style={{ display: "none" }} onChange={handleFileChange} />
                                        {filePreview && (
                                    ...
                                    <div className="mt-4 d-flex justify-content-end gap-1">
                                        ...
                                        <Button type="submit" className="text-white m10-left submitButton" text={isLoading ? "Submitting..." : "Submit"} disabled={isLoading} />
                                    </div>
                                </div>
                            </form>
                        </div>
