...
/* ── Design tokens (mirror Agency CSS vars) ── */
const T = {
  accent:  "#6366f1",
  accent2: "#a855f7",
...
const S: Record<string, React.CSSProperties> = {
  ...
  personCellClick: {
    display: "flex", alignItems: "center", gap: "12px", padding: "4px 0", cursor: "pointer",
  },
  ...
  cellText: { fontSize: "13px", color: T.txt, fontWeight: 500, whiteSpace: "nowrap" },
  cellDate: { fontSize: "12.5px", color: T.txtDim, fontWeight: 500, whiteSpace: "nowrap" },
  impressionWrap: { maxWidth: "260px", fontSize: "13px", color: T.txt, lineHeight: 1.5 },
  readMore: {
    background: "none", border: "none", padding: 0,
    fontSize: "12px", fontWeight: 600, color: T.accent, cursor: "pointer", marginLeft: "4px",
  },
  actionBtn: {
  ...
  paginationWrap: { padding: "16px 20px", borderTop: `1px solid ${T.border}` },
};
      ...
      Cell: ({ row, index }: any) => {
        ...
        return (
          <div style={S.impressionWrap}>
            {isExp ? text : text.substring(0, 50) || "—"}
            {text.length > 10 && (
      ...
      Cell: ({ row }: { row: SuggestedServiceData }) => (
        <button
          ...
          onMouseEnter={(e) => {
            ...
            b.style.background    = T.blSoft;
            b.style.borderColor   = T.blue;
