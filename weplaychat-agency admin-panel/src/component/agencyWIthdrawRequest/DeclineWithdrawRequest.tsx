...
const declineStyle = `
  ...
  .dcl-stat:hover { box-shadow: 0 4px 14px rgba(244,63,94,0.10); }
  ...
  .dcl-stat-icon-red    { background: rgba(244,63,94,0.09); }
  .dcl-stat-icon-amber  { background: rgba(245,158,11,0.10); }
  .dcl-stat-icon-purple { background: rgba(99,102,241,0.10); }

  .dcl-stat-info { display: flex; flex-direction: column; gap: 1px; }
  .dcl-stat-label {
  ...
  .dcl-info-btn:hover {
    ...
    box-shadow: 0 3px 10px rgba(244,63,94,0.13);
    transform: translateY(-1px);
  }
  .dcl-info-btn img { width: 16px; height: 16px; object-fit: contain; }
  ...
  .dcl-modal {
    ...
    width: 100%;
    max-width: 460px;
    box-shadow: 0 8px 40px rgba(244,63,94,0.13);
    overflow: hidden;
    outline: none;
    animation: dclFade .2s ease;
  }
  ...
  .dcl-modal-close:hover { background: rgba(255,255,255,0.32); }
  ...
  .dcl-modal-field { display: flex; flex-direction: column; gap: 4px; }
  ...
  .dcl-close-btn:hover { background: #eef0f8; border-color: #d0d4e8; }
`;
...
const DeclineWithdrawRequest = (props: any) => {
  ...
  const totalCoins = Array.isArray(data)
    ? data.reduce((s: number, r: any) => s + (r?.coin ?? 0), 0) : 0;
  const totalAmount = Array.isArray(data)
    ? data.reduce((s: number, r: any) => s + (r?.amount ?? 0), 0) : 0;

  const withdrawDeclinedtable = [
    {
      Header: "No",
      Cell: ({ index }: { index: number }) => (
        <span className="dcl-no">{(page - 1) * rowsPerPage + index + 1}</span>
      ),
    },
    {
      Header: "Unique Id",
      Cell: ({ row }: { row: any }) => (
        <span className="dcl-uid">{row?.uniqueId || "—"}</span>
      ),
    },
    {
      Header: "Agency",
      Cell: ({ row }: { row: any }) => {
        ...
        return (
          <div className="dcl-agency">
            ...
            />
            <span className="dcl-agency-name">{row?.agencyId?.name || "—"}</span>
          </div>
        );
      },
    },
    ...
    },
    {
      Header: "Info",
      Cell: ({ row }: { row: any }) => (
        <button className="dcl-info-btn" onClick={() => handleOpenInfo(row)} title="View Info">
  ...
  return (
    <>
      ...
      <div className="dcl-wrap">
        ...
        {/* ── Summary Strip ── */}
        <div className="dcl-summary">
          <div className="dcl-stat">
            <div className="dcl-stat-icon dcl-stat-icon-red">
            ...
            <div className="dcl-stat-info">
              <span className="dcl-stat-label">Total Declined</span>
              <span className="dcl-stat-value">{totalDeclinedWithdrawal ?? 0}</span>
            </div>
          </div>
          <div className="dcl-stat">
            <div className="dcl-stat-icon dcl-stat-icon-amber">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
            ...
            <div className="dcl-stat-info">
              <span className="dcl-stat-label">Total Coins</span>
              <span className="dcl-stat-value">{totalCoins}</span>
            </div>
          </div>
          <div className="dcl-stat">
            <div className="dcl-stat-icon dcl-stat-icon-purple">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
            ...
            <div className="dcl-stat-info">
              <span className="dcl-stat-label">Total Amount</span>
              <span className="dcl-stat-value">{totalAmount}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div className="dcl-modal">
            <div className="dcl-modal-head">
              ...
              <button className="dcl-modal-close" onClick={handleCloseInfo}>
                <i className="ri-close-line" />
              </button>
            </div>
            <div className="dcl-modal-body">
              <div className="dcl-modal-field">
                <span className="dcl-modal-label">Payment Gateway</span>
                <div className="dcl-modal-value">{infoData?.paymentGateway || "—"}</div>
              </div>
              {infoData?.paymentDetails &&
                Object.entries(infoData.paymentDetails).map(([key, value]: [string, any]) => (
                  <div className="dcl-modal-field" key={key}>
                    <span className="dcl-modal-label">{key || "—"}</span>
                    <div className="dcl-modal-value">{String(value) || "—"}</div>
                  </div>
                ))
              }
            </div>
            <div className="dcl-modal-footer">
              <button className="dcl-close-btn" onClick={handleCloseInfo}>Close</button>
            </div>
          </div>
        </div>
      </Modal>
